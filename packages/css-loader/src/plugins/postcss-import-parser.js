...
import { WEBPACK_IGNORE_COMMENT_REGEXP } from '../constants';
...
function parseNode(atRule, key, options) {
  ...
  const rawParams =
    atRule.raws && atRule.raws[key] && typeof atRule.raws[key].raw !== 'undefined'
      ? atRule.raws[key].raw
      : atRule[key];
        ...
        async OnceExit() {
          ...
          const resolvedAtRules = await Promise.all(
            parsedAtRules.map(async (parsedAtRule) => {
              const { atRule, requestable, needResolve, prefix, url, layer, supports, media } =
                parsedAtRule;
              ...
              if (needResolve) {
                ...
                const resolvedUrl = await resolveRequests(resolver, loaderContext.context, [
                  ...new Set([request, url]),
                ]);
